...
import {AlertCircle, Edit, Minus, Package, Plus, Search} from "lucide-react";
...
export default function Inventory() {
    ...
    const getStockStatus = (quantity) => {
        ...
        if (qty === 0) return {color: 'text-red-600', bg: 'bg-red-50', label: 'Out of Stock'};
        if (qty < 10) return {color: 'text-yellow-600', bg: 'bg-yellow-50', label: 'Low Stock'};
        ...
        <div className="min-h-screen bg-background p-6">
            <div className="max-w-7xl mx-auto">
                <div className="rounded-lg bg-card p-6 shadow-sm mb-6">
                    ...
                    {error && (
                        <div
                            className="mb-4 p-4 bg-red-950 border border-red-800 rounded-lg flex items-center gap-2 text-red-50">
                ...
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="space-y-4 mt-4">
                        ...
                        <Button
                            ...
                            disabled={!productForm.name || !productForm.categoryId || !productForm.currentPrice}
            ...
            <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="mb-4">
                        <p className="text-sm text-gray-600 mb-2">Product: <span
                            className="font-semibold">{selectedProduct?.productName}</span></p>
                        <p className="text-sm text-gray-600">Current Stock: <span
                            className="font-semibold">{selectedProduct?.quantity}</span></p>
                    </div>
            ...
            <Dialog open={showRemoveModal} onOpenChange={setShowRemoveModal}>
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Reference ID
                                (Optional)</label>
                            <Input
                                type="text"
            ...
            <Dialog open={showHistoryModal} onOpenChange={setShowHistoryModal}>
                <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="mb-4">
                        <p className="text-sm text-gray-600">Product: <span
                            className="font-semibold">{selectedProduct?.productName}</span></p>
                    </div>
                                    ...
                                    <div key={transaction.id} className="border border-gray-200 rounded-lg p-4">
                                        <div className="flex justify-between items-start mb-2">
                                                <span
                                                    className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium ${
                                                        transaction.transactionType === 'PURCHASE' || transaction.transactionType === 'INITIAL' ? 'bg-green-100 text-green-800' :
                                                            transaction.transactionType === 'SALE' ? 'bg-red-100 text-red-800' :
                                        ...
                                        <div className="grid grid-cols-3 gap-2 text-sm">
                                            <div>
                                                ...
                                                <span
                                                    className={`ml-1 font-semibold ${parseFloat(transaction.quantityChange) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                        {parseFloat(transaction.quantityChange) >= 0 ? '+' : ''}{parseFloat(transaction.quantityChange).toFixed(2)}
                                                    </span>
                                            </div>
                                            <div>
                                                <span className="text-gray-500">Before:</span>
                                                <span
                                                    className="ml-1 font-semibold">{parseFloat(transaction.quantityBefore).toFixed(2)}</span>
                                            </div>
                                            <div>
                                                <span className="text-gray-500">After:</span>
                                                <span
                                                    className="ml-1 font-semibold">{parseFloat(transaction.quantityAfter).toFixed(2)}</span>
                                            </div>
                                        </div>
                                        {transaction.notes && (
